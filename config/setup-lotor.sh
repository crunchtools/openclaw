...
echo "  2. Install the systemd units from the HOST CONFIG REPO, not from here."
echo "     Both units live in ${SERVICE_DIR}/config/ and are committed to the"
echo "     lotor /srv repo, per Constitution XIV. This repo deliberately does"
echo "     not carry a copy: two git homes for one deployed file is how you"
echo "     end up restoring the stale one over production."
echo "       install -m 0644 ${SERVICE_DIR}/config/openclaw.crunchtools.com.service /etc/systemd/system/"
echo "       install -m 0644 ${SERVICE_DIR}/config/signal-api.crunchtools.com.service /etc/systemd/system/"
